...
int serialBufferDelay = 10; // ms to wait inbetween bytes transferred (can be tuned)
...
void loop()
{
  ...
  if ((incomingDigits > 0) && (incomingDigits <= maxDigits))
  {
    ...
    // print the number
    Serial.print("The output number = ");
    Serial.println(output);
    Serial.println("------");
    // need control for maximum # allowed
    // reset numFill flag
    //numFill = false;
    //}
  ...
  /*
  else if (Serial.available > maxDigits)
  {
    for (int k = Serial.available(); k < maxDigits; k++)
    {
...
int parse(int numArray[], int numDigits)
{
  ...
  for (int j = 0; j < numDigits; j++)
  {
    ...
    outputNum += numArray[j] * pow(10, numDigits - 1 - j);
  }
  return outputNum;
}
